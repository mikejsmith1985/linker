...
func TestListQualifyingScansJoin(t *testing.T) {
	...
	cols := []string{
		...
		"salary_min", "salary_max", "description", "source_names", "original_url", "employer_website", "review_status", "review_reason", "discovered_at",
	}
	now := time.Now()
	mock.ExpectQuery("FROM match_results m JOIN job_openings o").
		...
		WillReturnRows(pgxmock.NewRows(cols).AddRow(
			...
			0, 0, "desc", []byte(`["adzuna"]`), "https://x", "https://acme.com", "new", "", now,
		))
